...
import { FieldGroup, FieldSelect } from '../FormFields';
...
const ODS_LIST: { id: number; name: string; complementar?: boolean }[] = [
  ...
  // ODS complementares do Portal LISA (além dos 17 oficiais da ONU)
...
// Mapa de sugestões por macroarea — espelha os seeds (0004_macroarea_ts.sql)
const SUGESTOES_MACROAREA: Record<
  string,
  { cnpq: string; extensao: string; ods: string }
> = {
  MAC1: { cnpq: 'Ciências Humanas; Ciências Sociais Aplicadas', extensao: 'Direitos Humanos e Justiça; Educação; Cultura', ods: '1, 4, 5, 10, 16' },
  MAC2: { cnpq: 'Ciências Sociais Aplicadas; Engenharias', extensao: 'Trabalho; Tecnologia e Produção', ods: '8, 9' },
  MAC3: { cnpq: 'Ciências da Saúde; Ciências Biológicas', extensao: 'Saúde', ods: '3' },
  MAC4: { cnpq: 'Ciências Agrárias; Ciências Biológicas; Ciências Exatas e da Terra', extensao: 'Meio Ambiente', ods: '6, 7, 11, 12, 13, 14, 15' },
  MAC5: { cnpq: 'Engenharias; Ciências Sociais Aplicadas', extensao: 'Tecnologia e Produção; Direitos Humanos e Justiça', ods: '9, 11' },
  MAC6: { cnpq: 'Ciências Humanas; Linguística, Letras e Artes', extensao: 'Educação; Cultura; Comunicação', ods: '4, 10' },
  MAC7: { cnpq: 'Ciências Agrárias; Ciências da Saúde', extensao: 'Meio Ambiente; Saúde; Trabalho', ods: '2, 3, 12' },
  MAC8: { cnpq: 'Ciências Sociais Aplicadas', extensao: 'Direitos Humanos e Justiça', ods: '16, 17' }
};

export function ClassificacaoCnpqOdsStep() {
  ...
  return (
    <div>
      <div className="mb-12">
        ...
        {sugestao && (
          <p className="mt-4 text-sm opacity-55 leading-relaxed max-w-[680px]">
      ...
      {/* Callout de sugestão baseado na macroarea */}
      {sugestao && (
        <div
          className="mb-10 px-5 py-4"
          style={{ border: '1px solid rgba(46,163,155,0.2)', background: 'rgba(46,163,155,0.04)' }}
        >
          <div className="text-[9px] font-bold uppercase tracking-[0.18em] text-accent-glow mb-2">
            Sugestão baseada na macroarea selecionada
          </div>
          <div className="text-[12px] opacity-60 leading-relaxed">
            <strong>CNPq:</strong> {sugestao.cnpq} &nbsp;·&nbsp;{' '}
            <strong>Extensão:</strong> {sugestao.extensao} &nbsp;·&nbsp;{' '}
            <strong>ODS:</strong> {sugestao.ods}
          </div>
        </div>
      )}
      ...
      <FieldGroup label="Objetivos de Desenvolvimento Sustentável (ODS)" required>
        ...
        <p className="mt-4 text-[10px] opacity-35 leading-relaxed">
          * ODS 18, 19 e 20 são objetivos complementares do Portal LISA, reconhecidos
          pela AGIR/UFF mas não incluídos nos 17 oficiais da ONU.
        </p>
      </FieldGroup>
    </div>
  );
