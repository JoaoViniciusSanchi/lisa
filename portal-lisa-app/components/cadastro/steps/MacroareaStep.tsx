...
// Dados das 9 categorias editoriais — hardcodadas para etapa 03
...
// Dados das 8 macroareas — espelha o seed do banco (0004_macroarea_ts.sql)
const MACROAREAS = [
  {
    codigo: 'MAC1',
  ...
  },
  {
    ...
    cnpq_sugerido: 'Ciências Agrárias; Ciências Biológicas; Ciências Exatas e da Terra',
...
export function MacroareaStep() {
  ...
  const setCategoria = (categoria: CategoriaEditorial) => {
    // Clique na categoria atual → deseleciona
    if (categoriaEditorialNome === categoria) {
      dispatch({ type: 'SET_CLASSIFICATION', key: 'categoriaEditorialNome', value: '' });
    } else {
      dispatch({ type: 'SET_CLASSIFICATION', key: 'categoriaEditorialNome', value: categoria });
    }
  };

  const setPrincipal = (codigo: MacroareaCodigo) => {
    // Clique na principal atual → deseleciona
  ...
  const secundariasAtMaximo =
    macroareasSecundariasCodigos.length >= 2;
    ...
    <div>
      <div className="mb-12">
        ...
        <p className="text-lg font-light leading-relaxed opacity-70 max-w-[680px]">
          Selecione a <strong>categoria editorial</strong> e a <strong>macroarea temática principal</strong> da sua
          experiência. Opcionalmente, adicione até duas macroareas secundárias.
        </p>
      </div>

      {/* Seção de Categoria Editorial */}
      <div className="mb-10 pb-10 border-b border-line">
        <div className="mb-6">
          <h3 className="font-display text-sm font-semibold text-warm-white mb-4">
            Categoria Editorial
          </h3>
            ...
            {CATEGORIAS_EDITORIAIS.map((categoria) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'text-left p-4 border transition-all duration-150 text-[12px] leading-relaxed',
                    isSelected
                      ? 'border-accent bg-accent bg-opacity-10 text-accent-glow'
                      : 'border-line-strong text-warm-white hover:border-accent'
                  )}
                >
                  {categoria}
                </button>
      ...
      {/* Seção de Macro Áreas */}
      <div className="mb-10">
        <h3 className="font-display text-sm font-semibold text-warm-white mb-6">
          Macroareas
        </h3>
      </div>

      {/* Grid de cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-line mb-10">
        {MACROAREAS.map((mac) => {
          ...
          // Desabilita secundária se já 2 selecionadas e esta não está entre elas,
          // ou se é a principal
          const secundariaDisabled =
            !isSecundaria &&
            (secundariasAtMaximo || isPrincipal);

          return (
            <div
              key={mac.codigo}
              className={cn(
                'relative bg-bg-elevated transition-all duration-200',
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  isPrincipal && 'bg-accent',
                  isSecundaria && !isPrincipal && 'bg-accent opacity-50',
              ...
              <div className="pl-5 pr-5 pt-5 pb-4">
                ...
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div />
                  {isPrincipal && (
                    <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-bg-base bg-accent px-2 py-0.5 flex-shrink-0">
                      Principal
                    </span>
                  )}
                  {isSecundaria && (
                    <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-accent border border-accent px-2 py-0.5 flex-shrink-0">
                      Secundária
                    </span>
                  )}
                </div>
                ...
                <h3
                  className={cn(
                    'font-display text-base font-semibold leading-snug mb-2 transition-colors',
                    isPrincipal && 'text-accent-glow',
                    isSecundaria && !isPrincipal && 'text-warm-white',
                    !isSelected && 'text-warm-white opacity-75'
                  )}
                >
                  {mac.nome}
                </h3>
                ...
                {/* Subtemas */}
                <p className="text-[12px] leading-relaxed opacity-50 mb-4">
                  {mac.inclui}
                </p>
                ...
                <div className="flex gap-2 flex-wrap">
                  <button
                    ...
                    className={cn(
                      'text-[10px] font-semibold uppercase tracking-[0.12em] px-3 py-1.5 border transition-all duration-150',
                      isPrincipal
                        ? 'border-accent bg-accent text-bg-base'
                        : 'border-line-strong text-warm-white hover:border-accent hover:text-accent-glow'
                    )}
                  >
                    {isPrincipal ? 'Principal ✓' : 'Marcar como principal'}
                  </button>

                  {/* Botão secundária — oculto se for a principal */}
                  {!isPrincipal && (
                    <button
                      type="button"
                      ...
                      className={cn(
                        ...
                        isSecundaria
                          ...
                          : 'border-line text-warm-white opacity-50 hover:opacity-100 hover:border-line-strong',
                        secundariaDisabled && !isSecundaria && 'pointer-events-none opacity-20'
                      )}
                    >
      ...
      {/* Contador de secundárias */}
      {macroareasSecundariasCodigos.length > 0 && (
        <p className="text-[11px] opacity-40 tracking-[0.08em] mb-6">
          {macroareasSecundariasCodigos.length} de 2 macroareas secundárias selecionadas
        </p>
      )}

      {/* Banner de correspondência sugerida — aparece ao selecionar a principal */}
      {principalData && (
        <>
          ...
          <div
            ...
            style={{
              ...
              border: '1px solid rgba(46, 163, 155, 0.25)'
            }}
          >
            <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-accent-glow mb-3">
              Correspondências sugeridas para {principalData.nome}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-[12px]">
              <div>
                <span className="opacity-40 block mb-1 font-semibold uppercase tracking-[0.1em] text-[9px]">
                  Área CNPq
                </span>
            ...
            <p className="text-[11px] opacity-40 mt-4 italic leading-relaxed">
              Nas próximas etapas você pode confirmar ou escolher outras combinações.
              Estas são apenas sugestões baseadas na macroarea selecionada.
            </p>
          </div>
        </>
      )}
